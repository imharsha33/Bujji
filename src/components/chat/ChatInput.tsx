...
export function ChatInput({ onSend, isLoading }: Props) {
  ...
  return (
    <div className="px-3 sm:px-4 pb-3 sm:pb-5 pt-1">
      {images.length > 0 && (
        <div className="flex gap-2 mb-2 flex-wrap">
          {images.map((src, i) => (
            <div key={i} className="relative group">
              <img src={src} alt="" className="h-14 w-14 sm:h-16 sm:w-16 rounded-xl object-cover border border-border" />
              <button
      ...
      <div className="relative rounded-2xl border border-border/50 bg-card input-glow transition-all duration-300">
        {/* Char count */}
        <div className="absolute top-2.5 right-3 text-[9px] sm:text-[10px] text-muted-foreground/50 tabular-nums">
          {text.length}/{MAX_CHARS}
        </div>

        <div className="pt-2.5 pb-1.5 px-3 sm:px-4">
          <Textarea
            value={text}
            onChange={(e) => handleChange(e.target.value)}
            onKeyDown={onKey}
            placeholder="Ask BUJJI anything..."
            className="min-h-[40px] sm:min-h-[60px] max-h-32 sm:max-h-40 resize-none border-0 bg-transparent focus-visible:ring-0 shadow-none text-sm p-0 placeholder:text-muted-foreground/40"
            rows={1}
          />
        </div>

        <div className="flex items-center justify-between px-2.5 sm:px-3 pb-2.5">
          ...
          <Button
            ...
            size="icon"
            className="rounded-xl h-8 w-8 sm:h-9 sm:w-9 bg-[hsl(var(--primary))]/20 text-[hsl(var(--primary))] hover:bg-[hsl(var(--primary))]/30 border border-[hsl(var(--primary))]/30 disabled:opacity-20 transition-all duration-200 active:scale-95"
          >
      ...
      <div className="hidden sm:flex items-center justify-center gap-3 mt-2 text-[10px] text-muted-foreground/40">
